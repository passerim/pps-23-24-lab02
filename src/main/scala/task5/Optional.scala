...
object Optionals:
  /** Optional is a type that represents a value that may or may not be present. Similar to Optional
    * in Java but using the ADT concept. Therefore, an Optional is a sum type with two cases: Maybe
    * and Empty. Maybe contains the value, and Empty represents the absence of a value.
    *
    * @tparam A
    */
  enum Optional[A]:
  ...
  object Optional:
    ...
    /** getOrElse returns the value of the optional if it is Maybe, otherwise it returns the default
      * value. Example: orElse(Maybe(1), 0) == 1 orElse(Empty(), 0) == 0
      *
      * @param optional
    ...
    /** map applies the function f to the value of the optional if it is Maybe, otherwise it returns
      * Empty. Example:
      *
      * map(Maybe(1), (x: Int) => x + 1) == Maybe(2) map(Empty(), (x: Int) => x + 1) == Empty()
      *
      * @param optional
      ...
      * @return
      *   the result of applying the function to the value of the optional if it is Maybe, otherwise
      *   Empty
      */
